...
public class PdfUtils {
    ...
    public static ByteArrayInputStream citiesReport(List<Student> student, User user) {
        ...
        try {
            ...
            for(Student prt : student) {
                ...
                double ppp1 = Double.parseDouble(totgp(prt.getEnglishTh()+ prt.getEnglishPr()));
                double ppp2 = Double.parseDouble(totgp(prt.getNepaliTh()+prt.getNepaliPr()));
                double ppp3 = Double.parseDouble(totgp(prt.getMathTh()));
                double ppp4 = Double.parseDouble(totgp(prt.getScienceTh()+prt.getSocialPr()));
                double ppp5 = Double.parseDouble(totgp(prt.getSocialTh()+prt.getSocialPr()));
                double ppp6 = Double.parseDouble(totgp(prt.getObtTh()+prt.getObtPr()));
                double ppp7 = Double.parseDouble(totgp(prt.getHealthTh()+prt.getObtPr()));
                double ppp8 = Double.parseDouble(totgp(prt.getMoralTh()+prt.getMoralPr()));
                double ppp9 = Double.parseDouble(totgp(prt.getOptTh()+prt.getOptPr()));
                DecimalFormat df=new DecimalFormat("#.##");
                double total1=ppp1+ppp2+ppp3+ppp4+ppp5+ppp6+ppp7+ppp8+ppp9;
                double total2=total1/9;
                String gpa=df.format(total2);
                float tutal=(prt.getEnglishTh()+prt.getEnglishPr()+prt.getNepaliTh()+prt.getNepaliPr()
                +prt.getMathTh()+prt.getScienceTh()+prt.getSciencePr()+prt.getSocialPr()+prt.getSocialTh()
                +prt.getObtTh()+prt.getObtPr()+prt.getMoralTh()+prt.getMoralPr()+prt.getOptTh()+prt.getOptPr())/8;
                System.out.println(tutal);
                String ag=gpa(total2);

                Image img1 = Image.getInstance("src/main/webapp/resources/images/4jugal.png");
                pdfWriter.getDirectContentUnder().addImage(img1, 78, 0, 0, 70, 35, 750);
                Image img2 = Image.getInstance("src/main/webapp/resources/images/box.png");
                ...
                Chunk blackText = new Chunk("RURAL MUNICIPALITY LEVEL EXAMINATION BOARD"
                        + "\n BASIC LEVEL EXAMINATION, "+prt.getYear(), black);
                Paragraph ppp = new Paragraph(blackText);

                ppp.setAlignment(Element.ALIGN_CENTER);
                document.add(ppp);

                Paragraph para = new Paragraph("Jugal Rural Municipality Bagmati Provience, Nepal");
                ...
                Chunk chunk1 = new Chunk("This is certify that Mr. | Miss.        " + prt.getName().toUpperCase() + "\nson|daughter of Mr.     "
                        + prt.getFatherName().toUpperCase() + "\nand Mrs.     " + prt.getMotherName().toUpperCase() + "\nthe student of  " +user.getSchoolName().toUpperCase()
                        +" "+user.getSchoolAddress().toUpperCase() +
                        "\nhas appeared in the Annual Examination of class Eight of sambat   " + prt.getYear()
                ...
                document.add(p22);
                document.add(new Paragraph("\n"));
                Paragraph pot = new Paragraph("Sym/Roll No.: " + prt.getRollNo() + ""
                        + "                                        MARK-SHEET");
                document.add(pot);
                ...
                table1.addCell(check(prcal(prt.getNepaliPr())));
                ...
                table1.addCell(check(prcal(prt.getSciencePr())));
                table1.addCell(check(prt.getScienceTh() + prt.getSciencePr()));
                table1.addCell(totgp(prt.getScienceTh() + prt.getSciencePr()));
                table1.addCell("");
                table1.addCell("5.");
                table1.addCell("Social Studies");
                table1.addCell(check(thcal(prt.getSocialTh())));
                table1.addCell(check(prcal(prt.getSocialPr())));
                ...
                table1.addCell(check(prt.getOptTh() + prt.getOptPr()));
